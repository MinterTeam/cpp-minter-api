...
class base_request : public minter::api::io_container {
public:
    ...
    /// \param icase
    /// \return
    std::optional<minter::api::kv> find_query(const std::string& key, bool icase = true) const;
#else
    minter::api::kv find_query(const std::string& key, bool icase = true) const;
#endif
    ...
    /// \param icase search case sensititvity
    void set_query(const minter::api::kv& kv, bool icase = true);
...
class request : public minter::api::base_request {
public:
    request()
        : base_request() {
    }
    ...
    //    void set_body(const minter::api::request_body& body);
    //    void set_body(minter::api::request_body&& body);
};
