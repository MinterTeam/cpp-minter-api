...
using kv_vector = std::vector<minter::api::kv>;
...
public:
    ...
    void set_header(minter::api::kv&& key_value);
    ...
    void set_headers(const minter::api::icase_map_t& map);

    /// \brief Adds from map new headers values, if some key exists, value will overwrited
    /// \see add_header(const KeyValue&)
    /// \param mmp
    void set_headers(const minter::api::icase_multimap_t& mmp);
    ...
    std::optional<minter::api::kv> find_header_pair(const std::string& name) const;
#else
    /// \brief Search for header and return row as pair: wss::web::KeyValue
    /// \param name string. Searching is case insensitive
    /// \return pair wss::web::KeyValue . Check "first" for emptyness if header not found (i'm lazy to write my own optional for now)
    minter::api::kv find_header_pair(const std::string& name) const;
#endif
    ...
    /// \param kv
    void add_header(const minter::api::kv& kv);
    ...
    const minter::api::kv_vector& get_headers() const;
...
protected:
    minter::api::kv_vector m_headers;
    std::string m_body;
};
